#!/usr/bin/env python3
"""
SKVQ QK DDR/HBM Memory Access Trace Generator
...
def addr_zpsc(args: Namespace, b: int, h: int, c: int, token_offset: int) -> int:
    """
    Compute byte address for ZPSC[b][h][channel // group_size][token_offset].
    Layout: ZPSC[batch][head][channel // group_size][token]
    """
    group_num = args.dim // args.group_size
    group_id = c // args.group_size
    token_base = (b * args.head * group_num * args.token_num + h * group_num * args.token_num + group_id * args.token_num) * 4
...
def generate_tiled_trace(args: Namespace, tile: int, b: int, h: int, channel_start: int, channel_end: int) -> Generator[Tuple[int, int ,int, bool, str], None, None]:
    ...
    for token_start in range(0, args.token_num, args.pe_num):
        ...
        for c in range(channel_start, channel_end):
            # --- 2. Load SKVQ ZPSC[b][h][tok_start:tok_end] ---
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="SKVQ QK DDR/HBM Memory Access Trace Generator",
