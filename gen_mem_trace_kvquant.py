#!/usr/bin/env python3
"""
KVQuant DDR/HBM Memory Access Trace Generator
...
per-channel量化, 每个channel有ZP,SC, 和LUT, 另外还有CSR格式的1% Outliers
"""
...
def addr_param(args: Namespace, h: int, c: int, param_idx: int) -> int:
    ...
    Layout: Params[head][channel][18], each element 16-bit (2 bytes).
    param_idx: 0=dc_init, 1=scale_init, 2-17=LUT (16)
    """
    ...
    for token_start in range(0, args.token_num, args.pe_num):
        ...
        for t in range(token_start, token_end):
            ...
            if ptr_addr not in param_cache:
                ...
                yield (tile, ptr_addr, 0, True, f"K_OUTLIER_PTR b={b} t={t}")
        ...
        if nnz_hi > nnz_lo:
            data_start_addr = burst_align(addr_outlier_data(args, b, nnz_lo), bs)
            data_end_addr   = burst_align(addr_outlier_data(args, b, nnz_hi - 1), bs)
            addr = data_start_addr
            # 和 K 合并
            while addr <= data_end_addr:
                if addr not in param_cache:
                    param_cache.add(addr)
                    yield (tile, addr, 0, True, f"K_OUTLIER b={b} t=[{token_start},{token_end})")
                addr += bs

        for c in range(channel_start, channel_end):
            # --- 3. Load K[b][h][c][tok_start:tok_end] ---
            k_start_addr = burst_align(addr_k(args, b, h, c, token_start), bs)
            k_end_addr = burst_align(addr_k(args, b, h, c, token_end - 1), bs)  # last element
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="KVQuant QK DDR/HBM Memory Access Trace Generator",
    ...
    parser.add_argument("--k-base", type=lambda x: int(x, 0), default=0x0500_0000)
    ...
    parser.add_argument("--outlier-data-base", type=lambda x: int(x, 0), default=0x0900_0000,
